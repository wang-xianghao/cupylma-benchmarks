...
def train_with_lma(model, train_loader, test_loader, num_epochs, slice_size, devices):
    ...
    for epoch in range(1, num_epochs + 1):
        for idx, (x, y) in enumerate(train_loader):
            ...
            # Print the result
            printf(f'Epoch {epoch} Batch {idx}: Average loss {test_loss:10.3e}, Accuracy {test_acc:10.3e}')
...
def train_with_adam(model, train_loader, test_loader, num_epochs, device):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        printf(f'Epoch {epoch}: Average loss {test_loss:10.3e}, Accuracy {test_acc:10.3e}')
...
def main():
    ...
    parser.add_argument('--out', type=str, default="result.csv", help='number of epochs')
    ...
    # Train
    if optim == 'adam':
        time_stamps, loss_values, acc_values = train_with_adam(model, train_loader, test_loader, num_epochs, devices[0])
    elif optim == 'lma':
        train_with_lma(model, train_loader, test_loader, num_epochs, slice_size, devices)
    else:
